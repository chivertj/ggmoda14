...
int main (int argc, char ** argv) {
  if (argc<4) {
    cout <<"Requires: list of images in a text file (provide text file name), output base filename, hit threshold (float)"<<endl;
    return -1;
  }
  HOGDescriptor hog;
  ...
  float hitThreshold=atof(argv[3]);
  ...
  while (File && imgcountidx--) {
    ...
    if (imagefilename!="" && imagefilename!=" " && imagefilename!="\n") {
      ...
      if (!img.empty()) {
        img.copyTo(colorop);
        vector<Rect> regionrects=hoghelpers::detectrects(hog,hitThreshold,img);
        ...
        cout <<"hoghelpers::showDetections"<<endl;
        hoghelpers::showDetections(regionrects,colorop);
